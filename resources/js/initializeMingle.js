...
export default function initializeMingle(Livewire, config) {
    ...
    Livewire.hook("component.init", async ({ component, cleanup }) => {
        // Get the component name and data from the element's attributes
        let component_name = component.el.dataset.mingleComponent;
        ...
        while (renderAttempts < config.maxRenderAttempts) {
            try {
                ...
                break; // Break out after successful render
            } catch (e) {
                renderAttempts++;
                await new Promise((resolve) =>
                    setTimeout(resolve, config.renderAttemptDelay)
                );
            }
        }
        ...
        if (renderAttempts >= config.maxRenderAttempts) {
            throw new Error(
                `MingleJs: Component '${component_name}' failed to render after ${config.maxRenderAttempts} attempts.`
            );
        }

        // Cleanup is called just before Livewire removes the element from the DOM
        cleanup(() => {
            window.Mingle.Elements[component.id].unmount();
            delete window.Mingle.Elements[component.id];
        });
    });
    ...
    Livewire.hook("morph.updated", ({ component, el }) => {
        ...
        mingle_element.updateProps({
            ...JSON.parse(el.dataset.mingleData),
            wire: component.$wire,
        });
    });
}
...
function createComponent(livewire_component, component_name) {
    ...
    if (!renderer) {
        throw new Error(
            `MingleJs: Renderer '${component.renderer}' not found.`
        );
    }
    ...
    if (!root_element) {
        throw new Error(
            `Mingle root element for component '${component_name}' not found.`
        );
    }

    // Parse the component props
    const component_props = JSON.parse(
        livewire_component.el.dataset.mingleData
    );
    ...
    try {
        ...
        window.Mingle.Elements[livewire_component.id] = {
            ...component_instance,
            props: livewire_component.el.dataset.mingleData, // Store stringified props for comparison, on morph.updated.
        };
    } catch (e) {
        throw new Error(
            `MingleJs: Failed to create component '${component_name}'. ${e.message}`
        );
    }
}
